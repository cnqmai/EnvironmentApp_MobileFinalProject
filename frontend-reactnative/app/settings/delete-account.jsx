...
const DeleteAccountScreen = () => {
  ...
  const handleDeleteAccount = () => {
    ...
    Alert.alert(
      "Xác nhận",
      "Bạn có chắc chắn muốn xóa tài khoản và TẤT CẢ dữ liệu cá nhân vĩnh viễn không?",
      [
        {
          text: "Hủy",
          style: "cancel",
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              // 1. Lấy thông báo lỗi từ Backend (nếu có)
              // Spring Boot thường để message trong error.response.data.message
              const backendMessage = error.response?.data?.message || error.response?.data || "";

              let displayMessage = error.message; // Mặc định là lỗi chung

              // 2. Ưu tiên hiển thị message từ backend
              if (typeof backendMessage === 'string' && backendMessage.includes("Mật khẩu không chính xác")) {
                  displayMessage = "Mật khẩu không chính xác. Vui lòng kiểm tra lại.";
              } else if (error.message.includes("401") || backendMessage.includes("Unauthorized")) {
                  displayMessage = "Mật khẩu không đúng hoặc phiên đăng nhập đã hết hạn.";
              } else if (backendMessage) {
                  // Nếu có lỗi khác từ server (ví dụ: Chuỗi xác nhận sai)
                  displayMessage = backendMessage;
              }

              Alert.alert("Lỗi Xóa Tài khoản", displayMessage);
      ...
      >
        ...
        <View style={styles.contentSection}>
          ...
          <Text style={styles.warningText}>
            Khi bạn xóa tài khoản, toàn bộ dữ liệu cá nhân, báo cáo, điểm thưởng
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          disabled={confirmText.toLowerCase() !== "xoataikhoan" || loading}
        >
          {loading ? (
